...
const getProducts = async function (req, res) {
  ...
  if(name){   
    ...
    filter['title']['$options'] = 'i'
     ...
     if (size1.map(x => validation.isValid(x)).filter(x => x === false).length !== 0) 
  ...
  if(style){
   ...
   if(installments){   
    ...
    if(!validation.isValid(availableSizes)) 
     {return res.status(400).send({status:false, message: "required availableSizes "})}
    
    sizes = availableSizes.split(',').map(a => a.trim().toUpperCase())
     let allSize=product.availableSizes 
     for(let i = 0; i < sizes.length; i++) 
     {
       if(!(["S", "XS","M","X", "L","XXL", "XL"] ).includes(sizes[i])){ 
             return res.send({status: false, message: "sizes should be from S,XS,M,X,L,XXL, XL"}) } 
      else if(allSize.includes(sizes[i])){
        continue} 
        else {allSize.push(sizes[i])} } 
        productUpdate.availableSizes = allSize } 
