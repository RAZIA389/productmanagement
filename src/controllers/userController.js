...
const createUser = async function (req, res) {
  try {
    ...
    if (!(email.trim()).match(/^[a-zA-Z_\.\-0-9]+[@][a-z]{3,6}[.][a-z]{2,4}$/)) 
    { return res.status(400).send({ status: false, message: 'invalid E-mail' }) };
...
const loginUser = async function (req, res) {
  try {
    if (!validation.isValidRequestBody(req.body))
     { return res.status(400).send({ status: false, message: "Please enter details in the request Body" }) }

    let userName = req.body.email;
    let password = req.body.password;
    if (!validation.isValid(userName)) 
    { return res.status(400).send({ status: false, message: "Please enter your email Address" }) }
    if (!(userName.trim()).match(/^[a-zA-Z_\.\-0-9]+[@][a-z]{3,6}[.][a-z]{2,4}$/)) 
    { return res.status(400).send({ status: false, message: 'invalid E-mail' }) };
...
const updateUserProfile = async function (req, res) {
  try {
    if (!validation.isValidRequestBody(req.body) && !(req.files))
     { return res.status(400).send({ status: false, message: "Please enter details in the request Body" }) }
    ...
    if (validation.isValidIncludes("address", req.body)) {
      ...
      if (validation.isValidRequestBody(address)) {
        if (validation.isValidIncludes("shipping", address1)) {
          if (!validation.isValid(address1.shipping)) 
